<?php
###########
## Módulo para montagem do relatório de Congelamento da Planilha de Foto e Vídeo em PDF
## Criado: 27/09/2009 - Maycon Edinger
## Alterado: 
## Alterações: 
##
###########
...
{    
  ...
  $this->Cell(0,6,"Planilha de Congelamento de Foto e Vídeo do Evento",0,0,"R");
  $this->Ln();	  
  $this->SetFont("Arial","B",13);
	$this->Cell(0,6,"Data do Congelamento: "  . DataMySQLRetornar($data_congela) . " as " . $hora_congela,0,0,"R");
  $this->SetFont("Arial","",9);
...
if ($TipoRelatorio == 1){
	...
	//Caso tiver
		...
		while ($dados_formando = mysql_fetch_array($sql_formando)){
		  ...
		  $pdf->SetFont("Arial","B",9);
			...
			$sql_produto = mysql_query("SELECT 
																	...
																	FROM eventos_fotovideo_congela item
																	LEFT OUTER JOIN categoria_fotovideo centro ON centro.id = item.item_id
																	WHERE item.evento_id = $EventoId
																	AND item.data_congela = '$data_congela'
																	AND item.hora_congela = '$hora_congela' 
			...
			while ($dados_item = mysql_fetch_array($sql_produto)){
			  ...
			  $pdf->SetFont("Arial","B",9);	
				...
				$total_comissao_formando = $total_comissao_formando + (((($dados_item["valor_venda"] * $dados_item["quantidade_venda"]) - $dados_item["valor_desconto"]) * ($dados_item["comissao"] / 100)) + $dados_item["bonus_comissao"]);
		...
		$pdf->Cell(00,4, number_format($total_comissao_formando, 2, ",", ".") ,1,0,"R");
      ...
      $pdf->Cell(14,4, round(($dados_total_venda / $dados_total_disponivel) * 100) . ' %',1,0,'R');
		...
		$pdf->Cell(20,4, number_format($total_geral_comissao_formando, 2, ",", "."),1,0,"R");
		$pdf->ln();
		$pdf->SetX(101);
		$pdf->SetFont("Arial","B",9);	
		...
		$pdf->Cell(20,4, number_format($total_geral_comissao_formando - $dados_evento[desconto_comissao], 2, ",", "."),1,0,"R");
...
//Caso for tipo 2
	...
	//Caso tiver
		...
		while ($dados_total = mysql_fetch_array($sql_total)){
			...
			$pdf->Cell(14,4, round(($dados_total["total_venda"] / $dados_total["total_disponivel"]) * 100) . " %",1,0,"R");
