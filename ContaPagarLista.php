<?php
###########
## Módulo para Listagem das Contas a Pagar
...
## 26/06/2007 - Implementado filtragem opcional por data
## 17/07/2007 - Incluído a opção de filtragem por sacado
## 19/07/2007 - Incluído a opção de filtragem por subgrupo
## 14/08/2007 - Incluído a opção de filtragem agrupada grupo, subgrpo e categoria
###########
...
switch ($_GET[TipoListagem]) 
{
	...
	case 1: 
		...
		$sql = "SELECT 
				...
				WHERE pag.empresa_id = '$empresaId' 
				AND pag.data_vencimento >= '$dataIni' 
				AND pag.data_vencimento <= '$dataFim' 
				$TextoSituacao 
				$WhereRegiao
				ORDER BY pag.data_vencimento, pag.descricao";
	...
	case 2: 
		...
		$sql = "SELECT 
				...
				LEFT OUTER JOIN eventos eve ON eve.id = pag.evento_id
				WHERE pag.empresa_id = '$empresaId' 
				AND pag.grupo_conta_id = '$grupoId' 
				$TextoSQLData 
				$TextoSituacao
				$WhereRegiao				
				ORDER BY pag.data_vencimento, pag.descricao";
	...
	case 5: 
		...
		$sql = "SELECT 
				...
				LEFT OUTER JOIN eventos eve ON eve.id = pag.evento_id
				WHERE pag.empresa_id = '$empresaId' 
				AND pag.tipo_pessoa = '$TipoPessoa' 
				AND pag.pessoa_id = '$PessoaId' 
				$TextoSQLData 
				$TextoSituacao 
				$WhereRegiao
				ORDER BY pag.data_vencimento, pag.descricao";
	...
	case 6: 
		...
		$sql = "SELECT 
				...
				LEFT OUTER JOIN eventos eve ON eve.id = pag.evento_id
				WHERE pag.empresa_id = '$empresaId' 
				AND pag.subgrupo_conta_id = '$subgrupoId' 
				$TextoSQLData 
				$TextoSituacao 
				ORDER BY pag.data_vencimento, pag.descricao";
	...
	<tr>
		<td style="PADDING-BOTTOM: 2px">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td>
						<input name='Button' type='button' class="button" id="Submit" accesskey='N' title="Nova Conta a Pagar [Alt+N]" value='Nova Conta a Pagar' onclick="window.location='sistema.php?ModuloNome=ContaPagarCadastra';" />
					</td>
					<td align='right'>
