<?php 
###########
## Módulo para Autenticação de Usuário e Controle de sessões
...
//Encripta a senha para comparação
...
if (mysql_num_rows($query) > 0) 
{
	...
	//Alimenta as variáveis com o valor do array  
	$login = $dados['login'];
	...
	//Monta a variável a data atual
	$data_login = date('Y-m-d'); 
	//Monta a variável a hora atual
	$hora_login = date('H:i');
	//Monta a variável do IP do usuario
	$ip_login = getenv("REMOTE_ADDR");
	
	//Grava na tabela de usuários os novos valores de data, hora e IP de acessso
	...
	//Monta e executa a query para pegar os dados da empresa que o usuário pertence
	...
	//Alimenta as variáveis com os dados vindos do array
  ...
  $_SESSION['WorkCrmLogin'] = array(
	...
	//Verifica se o usuário nao está bloqueado
	if ($dados[travado] == 1 OR $dados[ativo] == 0) 
	{
		
		header("location: login.php?Erro=Usuário com acesso bloqueado !&Solucao=Entre em contato com o administrador do sistema.");
	...
	
	{
		...
		//Executa a rotina de compensação de cheques do dia  
...

{
    ...
    header("location: login.php?Erro=Login de usuário ou senha Inválida !&Solucao=Verifique seus dados de acesso e tente novamente.");
