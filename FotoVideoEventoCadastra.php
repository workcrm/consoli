...
$sql_usuario = "SELECT nova_conta_receber FROM usuarios WHERE usuario_id = $usuarioId";													  													  
						...
						<?php
							...
							if($_POST['Submit'])
							{
								...
								for ($contador_for = 1; $contador_for <= $edtTotalChk; $contador_for++)
								{
									...
									$formando_data_venda = "edtDataVenda" . $_POST[$texto_formando];
									$formando_data_envio_lab = "edtDataEnvioLab" . $_POST[$texto_formando];
									$formando_data_retorno_lab = "edtDataRetornoLab" . $_POST[$texto_formando];
									$formando_data_entrega_cliente = "edtDataEntregaCli" . $_POST[$texto_formando];
									$formando_data_envio_cliente = "edtDataEnvioCli" . $_POST[$texto_formando];
									...
									$formando_data_venda = DataMySQLInserir($_POST[$formando_data_venda]);
									$formando_data_envio_lab = DataMySQLInserir($_POST[$formando_data_envio_lab]);
									$formando_data_retorno_lab = DataMySQLInserir($_POST[$formando_data_retorno_lab]);
									$formando_data_entrega_cliente = DataMySQLInserir($_POST[$formando_data_entrega_cliente]);
									$formando_data_envio_cliente = DataMySQLInserir($_POST[$formando_data_envio_cliente]);
								
									if ($formando_atual != $_POST[$texto_formando])
									{
                
										//Insere os detalhes específicos do formando
										$sql_insere_obs = "UPDATE eventos_formando SET 
																			obs_compra = '$_POST[$obs_formando]',
																			data_venda = '$formando_data_venda',
																			data_envio_lab = '$formando_data_envio_lab',
																			data_retorno_lab = '$formando_data_retorno_lab',
																			data_entrega_cliente = '$formando_data_entrega_cliente',
																			data_envio_cliente = '$formando_data_envio_cliente'
																			WHERE id = '$_POST[$texto_formando]'";	
                                    
										// echo $sql_insere_obs . "<br/>";	   							
     							
										//Insere os registros na tabela
										mysql_query($sql_insere_obs);
                 
										$formando_atual = $_POST[$texto_formando];
...
<?php
	...
	$sql_formandos = mysql_query("SELECT * FROM eventos_formando WHERE evento_id = $EventoId AND status < 3 ORDER BY nome"); 
...
<table width="100%" id="4" cellpadding="0" cellspacing="0" border="0" class="listView">
	...
	<tr height="24">
		<td bgcolor="<?php echo $cor_celula ?>" colspan="7" valign="bottom" style="padding-left: 8px">    				 	 
			<span style="font-size: 14px"><b>							 
				<b><?php echo $dados_formandos['nome'] . ' (' . $dados_formandos['id'] . ')' ?></b>
			</span>
		</td>
        <td bgcolor="<?php echo $cor_celula ?>" colspan="5" align="right" style="padding-right: 4px;">
			<?php
			  
				if ($dados_usuario["nova_conta_receber"] > 0)
				{
				
			?>
			<input name="EmiteContaReceber" type="button" class="button" title="Gera as contas a receber para este formando" value="Gerar Conta a Receber" style="width: 130px" onclick="wdCarregarFormulario('ContaReceberCadastra.php?headers=1&Origem=FV&EventoFVId=<?php echo $EventoId ?>&FormandoFVId=<?php echo $dados_formandos['id'] ?>','conteudo')" />
			<?php
				}
			  
			?>
                
            <input name="EmiteAr" type="button" class="button" title="Emite o formulário de AR para este formando" value="Emitir AR" style="width: 100px" onclick="abreJanela('./relatorios/FormularioAR.php?TipoPessoa=1&PessoaId=<?php echo $dados_formandos['id'] ?>')" />
        </td>						 
	</tr>
	...
	<tr height="16">
    	<td bgcolor="<?php echo $cor_celula ?>" valign="top">
		  	<div align="center">
		...
		<td bgcolor="<?php echo $cor_celula ?>" align="left" valign="top" bgcolor="#fdfdfd" class="currentTabList">
    	...
    	<td bgcolor="<?php echo $cor_celula ?>" align="center" valign="top" bgcolor="#fdfdfd" class="currentTabList">
			<?php
    	...
    	<td bgcolor="<?php echo $cor_celula ?>" align="center" valign="top" bgcolor="#fdfdfd" class="currentTabList" style="padding-right: 0px">
	...
	<?php
		...
		$total_geral_desconto = $total_geral_descontos + $dados_procura_item_evento['valor_desconto'];
		$total_geral_comissao_formando = $total_geral_comissao_formando + $total_comissao_formando;
							
		echo "<tr>
				<td bgcolor='$cor_celula' valign='top'>
					<span style='font-size: 12px' style='padding-left: 4px; padding-right: 0px'><b>Obs</span>
				</td>
				<td bgcolor='$cor_celula' colspan='2' style='padding-left: 4px; padding-right: 0px'>											 
				...
				<td bgcolor='$cor_celula' colspan='4' align='right' valign='top' style='padding-right: 0px'>
					<span style='font-size: 14px'><b>Total do Formando:</span>$texto_pendente
				</td>
				<td bgcolor='$cor_celula' align='right' valign='top' style='padding-right: 4px'>
					<span style='font-size: 14px; color: #990000'>
						<b>$formata_total_formando<b>
					</span>
				</td>
                <td bgcolor='$cor_celula'>
                    &nbsp;
                </td>
				<td bgcolor='$cor_celula' colspan='2' align='right' valign='top'>
					<span style='font-size: 14px'><b>Comissão:</span>
				</td>
				<td bgcolor='$cor_celula' align='right' valign='top' style='padding-right: 3px'>
					<span style='font-size: 14px; color: #990000'>
        ...
        <tr>
			<td bgcolor="<?php echo $cor_celula ?>" colspan="12" style="border-bottom: 1px dashed; padding-top: 3px; padding-bottom: 3px">
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                        <td width="66" style="padding-left: 3px;">
							Data Venda:
                        </td>
                        <td width="90">
							<?php
                          
								$valor = DataMySQLRetornar($dados_formandos[data_venda]);
                            
          						//Define a data do formulário
								$objData->strFormulario = "cadastro";  
								
								//Nome do campo que deve ser criado
								$objData->strNome = "edtDataVenda$dados_formandos[id]";
                        ...
                        <td width="90">
							<?php
								...
								$valor = DataMySQLRetornar($dados_formandos[data_envio_lab]);
							  
								//Define a data do formulário
								$objData->strFormulario = "cadastro";  
								//Nome do campo que deve ser criado
								$objData->strNome = "edtDataEnvioLab$dados_formandos[id]";
								$objData->strRequerido = false;
								//Valor a constar dentro do campo (p/ alteração)
                        ...
                        <td width="90">
							<?php
								...
								$valor = DataMySQLRetornar($dados_formandos[data_retorno_lab]);
                            
								//Define a data do formulário
								$objData->strFormulario = "cadastro";  
								//Nome do campo que deve ser criado
								$objData->strNome = "edtDataRetornoLab$dados_formandos[id]";
								$objData->strRequerido = false;
                        ...
                        <td width="90">
							<?php
								...
								$valor = DataMySQLRetornar($dados_formandos[data_entrega_cliente]);
                            
								//Define a data do formulário
								$objData->strFormulario = "cadastro";  
								//Nome do campo que deve ser criado
								$objData->strNome = "edtDataEntregaCli$dados_formandos[id]";
								$objData->strRequerido = false;
                        ...
                        <td>
							<?php
								...
								$valor = DataMySQLRetornar($dados_formandos[data_envio_cliente]);
                            
								//Define a data do formulário
								$objData->strFormulario = "cadastro";  
								//Nome do campo que deve ser criado
								$objData->strNome = "edtDataEnvioCli$dados_formandos[id]";
								$objData->strRequerido = false;
	      	 ...
	      	 <table border="0" cellpadding="0" cellspacing="0" width="100%">
					 ...
					 <?php
						...
						while ($dados_total = mysql_fetch_array($sql_total))
						{
							...
							$fecha_desconto = $fecha_desconto + $dados_total['total_desconto'];
						...
						<?php 
					 	...
					 	?>
							...
							<tr>
						 		<td colspan="5" align="right" style="padding-top: 10px; padding-right: 22px">
							 		<span style="font-size: 14px"><b>Total de Descontos:</span>
								</td>
								<td align="right" style="padding-top: 10px; padding-right: 10px">
									<span style="font-size: 14px; color: #990000"><b><?php echo number_format($fecha_desconto, 2, ',', '.') ?><b></span>
								</td>
								<td colspan="2">&nbsp;</td>
							</tr>
						 <tr>
