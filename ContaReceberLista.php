				...
				<tr>
					<td>
						<?php
							...
							{
						?>
						<input name="Submit" type="submit" class="button" title="Salva as alterações das contas a receber" value="Salvar Contas a Receber" />
						<?php
							}
						?>
					</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    ?>
      ...
      <td bgcolor="<?php echo $cor_celula ?>" style="border-bottom: 1px solid" height="20">
        <input name="<?php echo ++$edtItemChk ?>" type="checkbox" value="<?php echo $dados_rec[id] ?>" style="border: 0px" title="Clique para marcar ou desmarcar a alteração desta conta a receber"/>
      ...
      <?php
        ...
        if ($chkDataVencimento == 1)
        {
			...
			if ($dados_rec[situacao] == 1)
			{
				...
				$objData->strNome = "edtVencimento" . $edtItemChk;
				$objData->strRequerido = false;
				//Valor a constar dentro do campo (p/ alteração)
				$objData->strValor = DataMySQLRetornar($dados_rec[data_vencimento]);
				//Define o tamanho do campo 
				$objData->intTamanho = 10;
        ...
        if ($chkValorContrato == 1)
        {
          ...
          if ($dados_rec[situacao] == 1)
          {
              ...
              //CAMPO DE VALOR DO CONTRATO (ORIGINAL) 
							//Acerta a variável com o valor a alterar
							$valor_alterar = str_replace(".",",",$dados_rec[valor_original]);							
							
							//Cria um objeto do tipo WDEdit 
							$objWDComponente = new WDEditReal();
							
							//Define nome do componente
							$objWDComponente->strNome = "edtValorOriginal" . $edtItemChk;
							//Define o tamanho do componente
							$objWDComponente->intSize = 7;
        ...
        if ($chkValorBoleto == 1)
        {
          ...
          echo "<td bgcolor='$cor_celula' style='border-bottom: 1px solid; border-right: 1px dotted' align='center'>";           
			...
			if ($dados_rec[situacao] == 1)
			{
				...
				//Acerta a variável com o valor a alterar
				$valor_alterar = str_replace(".",",",$dados_rec[valor_boleto]);							
				
				//Cria um objeto do tipo WDEdit 
				$objWDComponente = new WDEditReal();
				
				//Define nome do componente
				$objWDComponente->strNome = "edtValorBoleto" . $edtItemChk;
				//Define o tamanho do componente
				...
				$objWDComponente->strEvento = "";
				//Define numero de caracteres no componente
				$objWDComponente->intMaxLength = 8;
        ...
        if ($chkValorMultaJuros == 1)
        {
			...
			if ($dados_rec[situacao] == 1)
			{
				...
				//Acerta a variável com o valor a alterar
				$valor_alterar = str_replace(".",",",$dados_rec[valor_multa_juros]);							
				
				//Cria um objeto do tipo WDEdit 
				$objWDComponente = new WDEditReal();
				
				//Define nome do componente
				$objWDComponente->strNome = "edtValorMultaJuros" . $edtItemChk;
				//Define o tamanho do componente
				$objWDComponente->intSize = 5;
				//Busca valor definido no XML para o componente
				$objWDComponente->strValor = "$valor_alterar";
