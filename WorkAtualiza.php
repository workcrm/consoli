...
$query = mysql_query("UPDATE parametros_sistema SET atualizando = 1");
...
//Recupera dos dados do evento
$sql_boleto = "SELECT * FROM boleto WHERE data_atualizacao = '$data_base'"; 
...
while ($dados_boleto = mysql_fetch_array($resultado_boleto))
{
	...
	//Verifica se o boleto ja existe no servidor remoto
	//Recupera dos dados do evento
	$sql_boleto = "SELECT id FROM WORK_boleto WHERE id = '$dados_boleto[id]'";  
	  
	//Executa a query
	$resultado_boleto = mysql_query($sql_boleto);

	//verifica o número total de registros
	$registros_boleto_id = mysql_num_rows($resultado_boleto);
	
	//Ser nao encontrou o boleto
	if ($registros_boleto_id == 0)
	{
	...
	//Caso exista o boleto, entao so o atualiza
	else
	
	{
	
		$sql_atualiza_boleto = mysql_query("UPDATE WORK_boleto SET
											id_hash = '$dados_boleto[id_hash]',
											prazo_pagamento = '$dados_boleto[prazo_pagamento]',
											taxa_boleto = '$dados_boleto[taxa_boleto]',
											valor_cobrado = '$dados_boleto[valor_cobrado]',
											valor_boleto = '$dados_boleto[valor_boleto]',
											...
											endereco1 = '$dados_boleto[endereco1]',
											endereco2 = '$dados_boleto[endereco2]',
											demonstrativo1 = '$dados_boleto[demonstrativo1]',
											demonstrativo2 = '$dados_boleto[demonstrativo2]',
											demonstrativo3 = '$dados_boleto[demonstrativo3]',
											instrucoes1 = '$dados_boleto[instrucoes1]',
											instrucoes2 = '$dados_boleto[instrucoes2]',
											instrucoes3 = '$dados_boleto[instrucoes3]',
											instrucoes4 = '$dados_boleto[instrucoes4]',
											boleto_recebido = '$dados_boleto[boleto_recebido]',
											data_recebimento = '$dados_boleto[data_recebimento]',
											valor_recebido = '$dados_boleto[valor_recebido]',
											obs_recebimento = '$dados_boleto[obs_recebimento]',
											evento_id = '$dados_boleto[evento_id]',
											formando_id = '$dados_boleto[formando_id]',
											reajustado = '$dados_boleto[reajustado]',
											data_reajuste = '$dados_boleto[data_reajuste]'
											WHERE id = '$dados_boleto[id]'");
...
//Marca no servidor que o sistema com a atualizacao OK
//Conecta ao banco de dados online
//Define a sting de conexão
$conexao = @mysql_connect($Server_atu,$Login_atu,$Senha_atu) or die('Nao foi possivel se conectar com o banco de dados do servidor de destino !');
