<?php
###########
## Módulo para Exibição dos Fornecedores pesquisados
...
if ($_GET["edtPesquisa"] != "") {
  $modo_vis = $_GET["edtVisualizar"];
	...
	//Monta a query
	//$consulta = "SELECT * FROM fornecedores WHERE nome LIKE '$pesquisa%' AND empresa_id = $empresaId ORDER BY nome";
...
//Se não, o usuário quer pesquisar por inicial da letra
  ...
  $modo_vis = $_GET["Modo_vis"];
	//Caso clicar no botáo para exibir todas as Fornecedores
	if ($filtragem == "todos") {
		//Monta a query pesquisando por empresa
	  //$consulta = "SELECT * FROM fornecedores WHERE empresa_id = $empresaId ORDER BY nome";
	  ...
	  //$consulta = "SELECT * FROM fornecedores WHERE nome LIKE '$filtragem%' AND empresa_id = $empresaId ORDER BY nome";		 
		...
		$consulta = "SELECT 
				  		  con.id,
						  	...
						  	FROM fornecedores con
						  	LEFT OUTER JOIN cidades cid ON cid.id = con.cidade_id
						  	WHERE con.nome LIKE '$filtragem%' AND con.empresa_id = $empresaId ORDER BY con.nome";
	}
    		...
    		if ($registros == 0) { 
      		echo "
        		...
        		<tr class='listViewThS1' height='20' background='image/fundo_consulta.gif'>
          		<td align='center'>&nbsp;&nbsp;Não foram encontrados fornecedores para a pesquisa !</td>
        		</tr>
        	</table>	
    			";
				} else {
      	...
      	echo "
        	...
        	<tr class='listViewThS1' height='20' background='image/fundo_consulta.gif'>
          	<td width='20'>";
							...
							if ($pc_rec > 1) {				
				  			//Exibe o botão
							echo "<a href='#' onClick=\"wdCarregarFormulario('ModuloFornecedoresLista.php?edtPesquisa=$_GET[edtPesquisa]&edtVisualizar=$_GET[edtVisualizar]&ChaveFiltragem=$_GET[ChaveFiltragem]&Modo_vis=$_GET[Modo_vis]&PaginaRec=$anterior_rec','retornopesquisa','1')\" title='Exibe a Página Anterior'>&nbsp;<img src='./image/bt_anterior.gif' alt='Exibe a Página Anterior' border='0' align='middle'></a></td><td width='50'><a href='#' onClick=\"wdCarregarFormulario('ModuloFornecedoresLista.php?edtPesquisa=$_GET[edtPesquisa]&edtVisualizar=$_GET[edtVisualizar]&ChaveFiltragem=$_GET[ChaveFiltragem]&Modo_vis=$_GET[Modo_vis]&PaginaRec=$anterior_rec','retornopesquisa','1')\" title='Exibe a Página Anterior'>Anterior</a></td>";
				  		}	 
				?>
						...
						<td align="center">
							<span class="pageNumbers">(Exibindo Fornecedores: <?php echo $conta_inicial_rec . " - " . $conta_final_rec . " de " . $tot_regs_rec ?>)</span>
						</td>
		    	...
		    	while ($dados_rec = mysql_fetch_array($limite_rec)){
					...
					//Monta o switch para a imagem da categoria do fornecedor
        ...
        if ($modo_vis == 1) {
          ...
          <table width='100%' cellpadding='0' cellspacing='0' border='0' class='listView'>
        		  ...
        		  <td class='listViewThS1'>
								&nbsp;&nbsp;<font size='3'><span style='color: #990000'>[$dados_rec[id]]</span> - <a title='Clique para exibir os detalhes deste fornecedor' href='#' onClick=\"wdCarregarFormulario('FornecedorExibe.php?FornecedorId=$dados_rec[id]','conteudo')\"><font size='3'>$dados_rec[nome]</font></a>
							</td>
			  		</tr>
        	    ...
        	    <td valign='top' bgcolor='#fdfdfd' class='oddListRowS1' style='padding: 0px' >
	  		  	...
	  		  	<tr height='16'>
	    				<td valign='middle' bgcolor='#fdfdfd' class='currentTabList' style='padding-right: 5px'>
	      		  	<div align='right'><a title='Clique para enviar um email para o fornecedor' href='mailto:$dados_rec[email]'>$dados_rec[email]</a></div>
	    				</td>
	    	  	</tr>	
	    		</table>
				";
  		  ...
  		  if ($modo_vis == 2) { 
	          ...
	          <tr>
	        	  <td align='left' class='listViewPaginationTdS1'><font size='2'><span style='color: #990000'><b>[$dados_rec[id]]</b></span> - <a title='Clique para exibir os detalhes deste fornecedor' href='#' onClick=\"wdCarregarFormulario('FornecedorExibe.php?FornecedorId=$dados_rec[id]','conteudo')\"><font size='2'>$dados_rec[nome]</font></a></td>
